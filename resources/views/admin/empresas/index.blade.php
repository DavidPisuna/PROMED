...
<div class="d-flex justify-content-between align-items-center">
    <h1 class="text-soft-primary"><i class="fas fa-building mr-2"></i>Gestión de Empresas</h1>
    <a href="{{ route('admin.empresas.create') }}" class="btn btn-soft-primary shadow-sm">
...
<div class="container-fluid">
    ...
    <div class="row mb-4">
        <div class="col-md-3">
            <div class="info-box bg-pastel-blue shadow-sm card-hover">
                <span class="info-box-icon bg-soft-primary"><i class="fas fa-building text-white"></i></span>
                <div class="info-box-content">
                    <span class="info-box-text text-muted">Total Empresas</span>
                    <span class="info-box-number text-dark">{{ $empresas->count() }}</span>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="info-box bg-pastel-green shadow-sm card-hover">
                ...
                <div class="info-box-content">
                    ...
                    <span class="info-box-number text-dark">{{ $empresas->where('activo', true)->count() }}</span>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="info-box bg-pastel-orange shadow-sm card-hover">
                ...
                <div class="info-box-content">
                    ...
                    <span class="info-box-number text-dark">{{ $empresas->where('activo', false)->count() }}</span>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="info-box bg-pastel-purple shadow-sm card-hover">
                <span class="info-box-icon bg-soft-info"><i class="fas fa-industry text-white"></i></span>
                <div class="info-box-content">
                    <span class="info-box-text text-muted">Actividades</span>
                    <span class="info-box-number text-dark">{{ $empresas->pluck('actividad_economica')->unique()->count() }}</span>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="col-md-8">
                    <div class="form-group">
                        <label class="form-label text-muted small">
                            <i class="fas fa-search mr-1"></i>Buscar Empresa
                        </label>
                        <div class="input-group input-group-pastel">
                            ...
                            <input type="text" name="table_search" class="form-control border-0" 
                                   placeholder="Buscar por nombre, RUC, actividad económica o representante...">
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
    ...
    {{-- Tabla de Empresas con Colores Suaves --}}
    <div class="card shadow-sm border-0">
        <div class="card-header bg-pastel-blue text-dark d-flex justify-content-between align-items-center border-0">
            <h5 class="card-title mb-0">
                <i class="fas fa-list mr-2 text-soft-primary"></i>Listado de Empresas
            </h5>
            <span class="badge badge-pastel-light text-dark py-2 px-3">
                <i class="fas fa-building mr-1 text-soft-primary"></i> {{ $empresas->count() }} registros
            </span>
        </div>
                ...
                <table class="table table-hover mb-0">
                    <thead class="bg-pastel-light">
                        <tr>
                            ...
                            <th class="border-0" style="width: 30%">Información de la Empresa</th>
                            <th class="border-0" style="width: 20%">Actividad Económica</th>
                            <th class="border-0" style="width: 17%">Representante Legal</th>
                    ...
                    <tbody>
                        ...
                        <tr class="{{ $empresa->activo ? '' : 'bg-pastel-light' }} table-row-hover" style="{{ !$empresa->activo ? 'opacity: 0.8;' : '' }}">
                            <td class="text-center">
                                <strong class="text-soft-primary">#{{ $empresa->id }}</strong>
                            </td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="mr-3">
                                        <i class="fas fa-building fa-2x text-soft-primary"></i>
                                    </div>
                                    <div>
                                        <strong class="d-block text-dark">{{ $empresa->nombre }}</strong>
                                        <div class="text-muted small">
                                            <i class="fas fa-id-card mr-1"></i>RUC: {{ $empresa->ruc }}
                                            @if($empresa->ciiu)
                                                <br><i class="fas fa-code mr-1"></i>CIIU: {{ $empresa->ciiu }}
                                            @endif
                                            <br><i class="fas fa-calendar-alt mr-1"></i>
                                            Registrada: {{ $empresa->created_at->format('d/m/Y') }}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <span class="badge badge-pastel-purple text-dark py-2 px-3">
                                    ...
                                    {{ Str::limit($empresa->actividad_economica, 30) }}
                                </span>
                            </td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <i class="fas fa-user-tie text-soft-warning mr-2"></i>
                                    <div>
                                        <strong class="text-dark">{{ $empresa->representante_legal }}</strong>
                                        @if($empresa->direccion)
                                            <br><small class="text-muted">
                                                <i class="fas fa-map-marker-alt mr-1"></i>
                                                {{ Str::limit($empresa->direccion, 25) }}
                                            </small>
                                        @endif
                                    </div>
                                </div>
                            ...
                            <td class="text-center">
                                <div class="d-flex flex-column align-items-center">
                                    <span class="badge {{ $empresa->activo ? 'badge-pastel-green' : 'badge-pastel-red' }} mb-1 py-2 px-3 text-dark">
                                        <i class="fas {{ $empresa->activo ? 'fa-check-circle' : 'fa-times-circle' }} mr-1"></i>
                                        {{ $empresa->activo ? 'ACTIVA' : 'INACTIVA' }}
                                    </span>
                                    <form action="{{ route('admin.empresas.toggle', $empresa) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-sm {{ $empresa->activo ? 'btn-outline-pastel-orange' : 'btn-outline-pastel-green' }}">
                                            <i class="fas {{ $empresa->activo ? 'fa-toggle-on' : 'fa-toggle-off' }} mr-1"></i>
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-pastel" role="group">
                                    ...
                                    <button type="button" 
                                            ...
                                            onclick="confirmDelete({{ $empresa->id }}, '{{ $empresa->nombre }}')">
                        ...
                        <tr>
                            <td colspan="6" class="text-center py-5 bg-pastel-light">
                                <div class="py-4">
                                    <i class="fas fa-building fa-4x text-soft-muted mb-3"></i>
                                    <h4 class="text-muted">No hay empresas registradas</h4>
                                    <p class="text-muted mb-4">Comience agregando la primera empresa al sistema</p>
                                    <a href="{{ route('admin.empresas.create') }}" class="btn btn-soft-primary">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <small class="text-muted">
                        ...
                        Mostrando <strong>{{ $empresas->count() }}</strong> empresas registradas
                    </small>
                </div>
                <div>
...
<script>
    function confirmDelete(empresaId, empresaNombre) {
        Swal.fire({
            title: '¿Eliminar Empresa?',
            html: `Esta acción no se puede deshacer<br>
                  <strong>${empresaNombre}</strong><br>
                  <small class="text-muted">ID: ${empresaId}</small>`,
            icon: 'warning',
